...
const cliMode = ref<'terminal' | 'chat'>('chat') // Default to chat since we have a session

const showAgentSelector = ref(false)

// Watch for mode changes to terminal - redirect to base /cli route
watch(cliMode, async (newMode) => {
  if (newMode === 'terminal') {
    // Switching to terminal mode should go to base /cli route
    await router.push('/cli')
  }
})
